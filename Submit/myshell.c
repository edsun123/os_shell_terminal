...
void signal_handler2(int signal){
    // int wstatus;
    // pid_t orphan_child;
    printf("ERROR: oof a seg fault, probably check your input? \n");
    exit(0);
}
...
int construct_commands(char* input, struct command* list,int width, int* struct_length){
    ...
    for (i=0; i<width; i++){
        ...
        if (input[i]=='|'){
            ...
            //printf("ERROR: unexpected symbol when constructing list \n");
        }
	...
	if (pid > 0) {//parent
        ...
        if (strcmp(arg[0], "cd")==0 || strcmp(arg[0], "exit")==0){
            ...
            if( execvp(arg[0], arg)<0){
                printf("ERROR: does not recognize command \n");
                exit(EXIT_FAILURE);
            }
        } 
    }
...
int launch_back(char** arg, int in) {
	...
	if (pid > 0) {//parent
        ...
        if (strcmp(arg[0], "cd")==0 || strcmp(arg[0], "exit")==0){
            ...
            if( execvp(arg[0], arg)<0){
                printf("ERROR: cannot perform basic exec \n");
                exit(EXIT_FAILURE);
            }
        } 
    }
...
int launch_prog(struct command* list, int struct_length){
    ...
    while(counter< struct_length){
        ...
        if (list[counter].pipe==1){
            ...
            err=pipe_connect (in, fd [1], (list+counter)->argv);
            ...
            pipe(fd);
            err=redirect_out(in, fd[1], (list+counter)->argv, (list+counter+1)->argv[0]);
            ...
            pipe(fd);
            err=redirect_in(in, fd[1], (list+counter)->argv, (list+counter+1)->argv[0]);
            close(fd[1]);
            in =fd[0];
            ...
            err=launch_back((list+counter)->argv, in);
        }
        else{
            err=execute_com((list+counter)->argv, in);
        }
